...
interface Props {
  habit?: Habit; // wenn vorhanden = Edit-Modus
...
export function HabitModal({ habit, onSave, onClose }: Props) {
  ...
  const handleSubmit = () => {
    if (!name.trim()) return;
    onSave({ name: name.trim(), color, description: description.trim(), category: category.trim(), frequency });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full sm:max-w-md bg-slate-800 rounded-t-3xl sm:rounded-2xl p-6 border border-slate-700 shadow-2xl max-h-[92vh] overflow-y-auto">
        {/* Zieh-Griff */}
        ...
        <div className="mb-4">
          <label className="block text-sm text-slate-400 mb-2">
            Beschreibung <span className="text-slate-600">(optional)</span>
          </label>
          <textarea
            value={description}
        ...
        <div className="mb-4">
          <label className="block text-sm text-slate-400 mb-2">
            Kategorie <span className="text-slate-600">(optional)</span>
          </label>
          <input
            type="text"
            value={category}
            onChange={(e) => setCategory(e.target.value)}
            placeholder="z.B. Gesundheit, Lernen, Sport…"
            className="w-full bg-slate-700 text-slate-100 placeholder-slate-500 rounded-xl px-4 py-3 border border-slate-600 focus:border-indigo-500 focus:outline-none transition-colors"
          />
        </div>
        ...
        {/* Häufigkeit */}
        <div className="mb-5">
          <label className="block text-sm text-slate-400 mb-2">Häufigkeit</label>
        ...
        {/* Farbe */}
        <div className="mb-7">
          <label className="block text-sm text-slate-400 mb-3">Farbe</label>
        ...
        <button
          ...
          disabled={!name.trim()}
          className="w-full py-3.5 rounded-xl font-semibold text-white transition-all active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
