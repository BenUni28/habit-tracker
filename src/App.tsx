      ...
      <div className="max-w-5xl mx-auto px-4 pb-28">
        ...
        <div className="pt-10 pb-6 flex items-start justify-between">
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <button onClick={signOut} className="p-2.5 text-slate-500 hover:text-slate-300 hover:bg-slate-800 rounded-xl border border-slate-700 transition-colors" title="Abmelden">
              <LogOut size={16} />
            </button>
          </div>
        </div>
        ...
        {error && (
          <div className="mb-5 flex items-start gap-3 p-4 bg-red-950/50 border border-red-800/50 rounded-2xl text-red-400">
            ...
            <div><p className="font-medium text-sm">Verbindungsfehler</p><p className="text-xs mt-0.5 text-red-500">{error}</p></div>
          </div>
        )}
          ...
          <div className="mb-6 bg-slate-800 rounded-2xl p-5 border border-slate-700">
            <div className="flex justify-between items-center mb-3">
              <div>
                ...
                {allDone && <span className="ml-3 text-emerald-400 text-sm font-medium">Alle erledigt! 🎉</span>}
              </div>
              <span className="text-2xl font-bold text-slate-100">{completedToday}<span className="text-slate-500 font-normal text-lg">/{habits.length}</span></span>
            </div>
        ...
        {view === 'stats' && <div className="max-w-2xl mx-auto"><StatsView habits={habits} completions={completions} getStreak={getStreak} getBestStreak={getBestStreak} getCompletionRate={getCompletionRate} getHeatmapData={getHeatmapData} today={today} /></div>}
      </div>
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-slate-900/95 backdrop-blur-md border-t border-slate-800">
        <div className="max-w-5xl mx-auto flex">
          {NAV.map(({ id, label, icon: Icon }) => (
            <button key={id} onClick={() => setView(id)} className={`flex-1 flex flex-col items-center gap-1 py-3 transition-colors ${view === id ? 'text-indigo-400' : 'text-slate-600'}`}>
              <Icon size={20} />
